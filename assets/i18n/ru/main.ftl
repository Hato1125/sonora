...
login-browser-detail = Sonora прочитает сессию YouTube Music из выбранного браузера.
login-already-connected = Уже подключено
login-add-title = Добавить сервис
login-add-detail = Подключите ещё один стриминговый сервис, чтобы переключаться между ними
...
settings-provider-none = Вход не выполнен
settings-provider-guest = { $provider } (гость)
settings-provider-detail = Сервис, в который выполнен вход на этом устройстве
settings-provider = Стриминговый сервис
settings-provider-add = Добавить ещё…
settings-account-detail = Выйти из аккаунта на этом устройстве
